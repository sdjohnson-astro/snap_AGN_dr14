...
parser = argparse.ArgumentParser(description='FG quasar checking. Once the interface is started, go forward/back by typing n or b. Classify as accepted, rejected, or uncertain by taping a, r, or u')
...
class checkforeground:
   """Definiting the checkforeground GUI for running through foreground galaxy
   ...
   def set_spec(self):
      ...
      self.filename = '../spectra/fg/spec-{}-{}-{}.fits'.format(self.pair['PLATE_FG'], self.pair['MJD_FG'], self.pair['FIBERID_FG'])
   ...
   def draw(self):
      ...
      if self.pair['QUALITY_FG'] == -1:
         qualitystring = '?'  
      if self.pair['QUALITY_FG'] == 0:
         qualitystring = 'rejected'  
      if self.pair['QUALITY_FG'] == 1:
         qualitystring = 'accepted'
      if self.pair['QUALITY_FG'] == -999:
         qualitystring = 'uncertain'                  
      
      self.plot_spec.setTitle('foreground index={} of {}: {}-{}-{}   z={:0.4f} quality={}'.format(self.index, self.N,
                                                                                        self.pair['PLATE_FG'],
                                                                                        self.pair['MJD_FG'],
   ...
   def setquality(self, quality):
      ...
      if quality == '?':
         self.pair['QUALITY_FG'] = -999
      
      if quality == 'accept':
         self.pair['QUALITY_FG'] = 1
      
      if quality == 'reject':
         self.pair['QUALITY_FG'] = 0
...
checker = checkforeground(args.i, args.xsize, args.ysize)